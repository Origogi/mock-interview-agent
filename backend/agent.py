...
SESSION_SCOPE_RULES = {
    "cs_fundamentals": {
        "focus": (
            "자료구조/알고리즘, 시간·공간 복잡도, 운영체제의 스레드·메모리·동시성, "
            "네트워크 프로토콜·지연·재시도, 데이터베이스 인덱스·트랜잭션·격리수준, 캐시 일관성 같은 CS 원리"
        ),
        "out_of_scope": (
            "Android/Flutter/React/Spring/FastAPI 같은 프레임워크 API, 라이프사이클, 화면 구성, "
            "상태 관리 라이브러리, DI/ORM 사용법 자체"
        ),
        "reframe": (
            "이력서가 Android나 특정 프레임워크 중심이어도 Activity/Fragment/Compose/ViewModel 사용법을 묻지 말고, "
            "메인 스레드 블로킹, 스케줄링, 네트워크 지연, 로컬 DB 인덱스·트랜잭션, 캐시 일관성처럼 CS 원리로 변환하세요."
        ),
    },
    "framework_usage": {
        "focus": (
            "프레임워크·라이브러리의 선택 이유, 라이프사이클/API 이해, 상태 관리, 프로젝트 구조, "
            "테스트 전략, 성능 최적화, 운영 중 한계와 대안"
        ),
        "out_of_scope": (
            "순수 CS 개념 설명만 요구하는 질문, 장애 대응 절차만 묻는 질문, 협업 커뮤니케이션만 평가하는 질문"
        ),
        "reframe": (
            "CS 원리는 보조 맥락으로만 사용하고, 질문의 핵심은 반드시 프레임워크 선택·적용·한계 인식이어야 합니다."
        ),
    },
    "problem_solving": {
        "focus": (
            "장애·성능 저하·설계 문제를 발견하고, 가설 수립, 계측, 원인 검증, 해결안 비교, "
            "재발 방지까지 구조적으로 해결하는 과정"
        ),
        "out_of_scope": (
            "프레임워크 API 사용법 퀴즈, 순수 CS 정의 질문, 팀 커뮤니케이션 방식만 평가하는 질문"
        ),
        "reframe": (
            "특정 기술 스택은 문제 상황의 배경으로만 쓰고, 질문은 원인 분석과 해결 프로세스를 평가하도록 구성하세요."
        ),
    },
    "communication": {
        "focus": (
            "기술적 의사결정 공유, 요구사항 조율, 피드백 수용, 갈등 해결, 비기술 이해관계자 설득, 회고와 개선"
        ),
        "out_of_scope": (
            "구현 세부사항을 깊게 파고드는 기술 질문, 프레임워크 API 질문, 순수 CS 지식 검증 질문"
        ),
        "reframe": (
            "기술 소재는 상황 설명으로만 사용하고, 질문의 핵심은 소통 방식, 합의 과정, 의사결정 근거 공유여야 합니다."
        ),
    },
}
...
def format_session_scope_rule(session_id: str) -> str:
    rule = SESSION_SCOPE_RULES[session_id]
    return (
        f"- 이번 세션의 주 평가대상: {rule['focus']}\n"
        f"- 이번 세션에서 금지할 질문 중심: {rule['out_of_scope']}\n"
        f"- 이력서/이전 답변 재해석 규칙: {rule['reframe']}"
    )
...
def format_interviewer_system(
    ...
    return INTERVIEWER_SYSTEM.format(
        ...
        session_scope_rule=format_session_scope_rule(meta["sessionId"]),
    )
...
[세션 경계]
세션 경계는 하드 바운더리입니다. 이력서나 이전 답변에 다른 세션의 소재가 있더라도, 현재 질문은 반드시 현재 세션의 평가대상 안에서만 구성하세요.
{session_scope_rule}

[전체 세션 구분]
- CS Fundamentals: CS 원리 검증 세션입니다. 프레임워크·플랫폼 API 지식 질문을 중심으로 삼지 마세요.
- Framework Usage: 프레임워크·라이브러리 적용 역량 검증 세션입니다. 순수 CS 퀴즈를 중심으로 삼지 마세요.
- Problem Solving: 문제 발견·원인 분석·해결 프로세스 검증 세션입니다. API 사용법이나 개념 정의를 중심으로 삼지 마세요.
- Communication: 기술 커뮤니케이션 검증 세션입니다. 구현 세부 지식 검증을 중심으로 삼지 마세요.

[원칙]
1. 단순 개념 질문보다 경험 기반 질문을 하되, 경험 소재는 반드시 현재 세션의 주 평가대상 안에서만 재해석하세요.
2. 한 번에 하나의 질문만 하세요.

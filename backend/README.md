...
| `generate_sample_answer` | `thread_id: str`, `quality_tier: str` ("best"\|"good"\|"bad") | `dict` (`{"answer": str, "expected_score_range": [int, int]}`) | 현재 진행 중인 면접 질문에 대해 등급별 샘플 답변 생성 (디버그/테스트용) |

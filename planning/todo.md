...
> **🔜 Next Up:** **M2 빈 답변/질문 가드** — M9 타임머신 완료 후 안정화 항목으로 진행.
...
| **5-14** | 목 | **[x] M9 타임머신 기획 확정 + 도메인 구현 플랜** | 완료 | PM + Design + FE + BE |
| **5-14** | 목 | **[x] M9 타임머신 MVP 구현** | 완료 | FE + BE + Design |
| **5-16~5-17** | 토~일 | **M2 빈 답변/질문 가드** | M9 이후 안정화 | BE |
| **5-17~5-18** | 일~월 | **M3 에러 표준화 + thread_id 충돌** | M9 이후 안정화 | BE |
| **5-10** | 토 | **[x] M4 SSE 토큰 스트리밍** | 완료 | BE + FE |
| **5-11~5-12** | 일~월 | **[x] M5 Page 3 메시지 자동 스크롤** | 1일 | FE |
| **5-18** | 월 | 버퍼/회귀 테스트 (M9 되감기 / M8 자연 종료 / F-29 조기 종료 회귀) | 0.5일 | All |
...
- [x] **M9: 타임머신 — 질문 시점 되감기 & 재답변** → **[features.md M9 / F-30]** / **완료: 2026-05-14** — 구현 완료 및 검증 통과
- [ ] **M2: 빈 답변 / 빈 질문 가드** → **[features.md M2]** / **M9 이후 안정화**
- [ ] **M3: 에러 응답 표준화 + thread_id 충돌 방지** → **[features.md M3]** / **M9 이후 안정화**
...
- [x] Page 3 평가 토스트를 3단계 copy/color로 통일 (현재 `>=7` vs `<7` binary)
- [x] Page 3 좌측 평가 카드 점수 배지를 Best/Good/Bad 3단계 tone으로 통일
- [x] 면접관 아바타 반응 기준을 canonical tier와 맞춤 (현재 `>=8`, `>=6` 기준)
- [x] tier helper 함수/상수 도입 검토로 토스트·카드·아바타 threshold 중복 제거
- [ ] 라이브 검증: F-28 샘플 답변 Best/Good/Bad 생성 → evaluator 점수 → UI 톤이 일관되게 매칭되는지 확인

---

### 7.6 F-30 타임머신 — 질문 시점 되감기 & 재답변 → **[features.md M9 / F-30]**
> **우선순위:** Must 최우선 — 데모에서 "실제 면접 후 후회"를 바로 학습 행동으로 바꾸는 핵심 wow 포인트.
> **상태:** 완료 — 구현 완료 및 검증 통과 (2026-05-14)
> **구현 방식:** 별도 페이지가 아닌 전역 full-cover 타임머신 오버레이. Page 4에서는 오버레이 뒤에서 Page 3으로 라우팅.

**완료 작업**
- [x] PM: M9 구현 플랜 작성 — 파괴적 되감기 범위, 상태 정책, BE/FE 인터페이스 가합의
- [x] DESIGN: full-cover 타임머신 오버레이 사양 — 시계 반시계 회전 + 질문 번호 역재생 + reduced-motion
- [x] BE: 선택 질문 이후 evaluation/report 무효화 방식 검토, 되감기 API 또는 기존 state mutation 경로 제안
- [x] FE: Page 3 질문 카드 버튼, Page 4 상세 피드백 버튼, 확인 모달, 오버레이 라우팅 흐름 설계

**MVP 체크리스트**
- [x] Page 3 완료된 질문 카드에 `답변 전으로 되감기` 버튼 노출
- [x] Page 4 상세 문항 피드백에 `이 질문 다시 답변하기` 버튼 노출
- [x] 확인 모달: 선택 질문 이후 답변/평가/리포트 무효화 안내
- [x] full-cover 오버레이: `타임머신 실행 중` + `Qn 답변 전으로 되감고 있어요`
- [x] Page 4에서 실행 시 오버레이 뒤에서 Page 3 라우팅
- [x] Qn으로 되감으면 Q1~Q(n-1)만 유지, Qn 이후 타임라인 삭제
- [x] 스트리밍/평가/조기 종료 모달 중 실행 방지
- [x] 되감기 후 새 답변 제출 → 이후 질문/리포트 새 타임라인 기준 생성

**검증**
- [x] `uv run pytest`, `npm run build`, `git diff --check`, Browser smoke 통과

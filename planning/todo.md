...
> **🔜 Next Up:** **M9 / F-30 타임머신 — 질문 시점 되감기 & 재답변** 구현 플랜 작성 디스패치 — Plan First, 코드 변경 X. 데모 임팩트 우선으로 M2/M3보다 먼저 진행. 목표 시작일: **2026-05-15 (금)**.
...
| **5-15** | 금 | **M9 타임머신 기획 확정 + 도메인 구현 플랜** | 0.5일 | PM + Design + FE + BE |
| **5-15~5-16** | 금~토 | **M9 타임머신 MVP 구현** | 1~1.5일 | FE + BE + Design |
...
- [ ] **M9: 타임머신 — 질문 시점 되감기 & 재답변** → **[features.md M9 / F-30]** / **최우선: 2026-05-15 시작 예정**
...
**내일 첫 작업**
- [ ] PM: M9 구현 플랜 작성 — 파괴적 되감기 범위, 상태 정책, BE/FE 인터페이스 가합의
- [ ] DESIGN: full-cover 타임머신 오버레이 사양 — 시계 반시계 회전 + 질문 번호 역재생 + reduced-motion
- [ ] BE: 선택 질문 이후 evaluation/report 무효화 방식 검토, 되감기 API 또는 기존 state mutation 경로 제안
- [ ] FE: Page 3 질문 카드 버튼, Page 4 상세 피드백 버튼, 확인 모달, 오버레이 라우팅 흐름 설계

**MVP 체크리스트**
- [ ] Page 3 완료된 질문 카드에 `답변 전으로 되감기` 버튼 노출
- [ ] Page 4 상세 문항 피드백에 `이 질문 다시 답변하기` 버튼 노출
- [ ] 확인 모달: 선택 질문 이후 답변/평가/리포트 무효화 안내
- [ ] full-cover 오버레이: `타임머신 실행 중` + `Qn 답변 전으로 되감고 있어요`
- [ ] Page 4에서 실행 시 오버레이 뒤에서 Page 3 라우팅
- [ ] Qn으로 되감으면 Q1~Q(n-1)만 유지, Qn 이후 타임라인 삭제
- [ ] 스트리밍/평가/조기 종료 모달 중 실행 방지
- [ ] 되감기 후 새 답변 제출 → 이후 질문/리포트 새 타임라인 기준 생성

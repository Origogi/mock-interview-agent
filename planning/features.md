...
| M9 | [~] **타임머신 — 질문 시점 되감기 & 재답변** (F-30) 🔄 (세션: FE-M9-tm-ux, since: 2026-05-15 00:17) | BE / FE / Design / Feature | **완료: 2026-05-14.** LangGraph checkpointer 기반 B-Lite snapshot 복원으로 특정 질문의 답변 전 상태로 되감아 재답변한다. Page 3 질문 카드와 Page 4 상세 피드백 진입, 확인 모달, 전역 타임머신 오버레이까지 구현 완료. |
...
### F-30. 타임머신 — 질문 시점 되감기 & 재답변 [구현 완료]
**왜**: 실제 면접이 끝난 뒤 가장 강한 학습 동기는 "아, 이 질문에는 이렇게 대답할걸"이라는 후회다. 타임머신은 이 감정을 서비스의 핵심 행동으로 바꿔, 특정 질문의 답변 전 상태로 되감고 다시 답변하게 한다.

**우선순위**
- **M9 Must 최우선**: 데모에서 기능 가치를 10초 안에 보여주는 wow 포인트라 M2/M3 안정화보다 먼저 진행.
- 데모 MVP는 범위를 좁힌다. 원본/새 답변 비교, 여러 타임라인 저장, 되감기 취소, 히스토리 복구는 제외.

**네이밍 원칙**
- 기능명/백로그명: **타임머신**
- 동작명: **되감기**
- Page 3 버튼/툴팁: **답변 전으로 되감기**
- Page 4 버튼: **이 질문 다시 답변하기**
- 모달 제목: **이 질문으로 되감기**
- 오버레이 타이틀: **타임머신 실행 중**

**MVP UX 흐름**
1. Page 3 좌측 사이드 메뉴의 완료된 질문/평가 카드에서 `되감기` 버튼 클릭.
2. 확인 모달 표시: "Q3 답변 전으로 돌아갑니다. 이 질문 이후의 답변과 평가는 다시 생성됩니다."
3. 전역 full-cover `타임머신 오버레이` 표시.
4. 오버레이가 떠 있는 동안 선택 질문의 답변 전 상태로 state 되감기.
5. 오버레이가 사라지면 Page 3에 선택 질문이 현재 질문으로 표시되고 입력창은 비어 있음.
6. 사용자가 새 답변 제출 → 이후 꼬리 질문/평가/리포트는 새 타임라인 기준으로 재진행.

**Page 4 라우팅 결정**
- 별도 타임머신 페이지는 만들지 않는다.
- Page 4 상세 문항 피드백의 `이 질문 다시 답변하기` 클릭 → 확인 모달 → 먼저 full-cover 오버레이 표시 → 오버레이 뒤에서 Page 3으로 라우팅 → state 되감기 완료 → 오버레이 fade out.
- 핵심은 Page 3으로 먼저 이동해서 화면 점프를 보이는 것이 아니라, 오버레이로 화면을 덮은 상태에서 라우팅과 state 변경을 처리하는 것.

**이펙트 방향**
- 시계가 반시계 방향으로 빠르게 되감기는 애니메이션을 기본 콘셉트로 한다.
- 보조 연출: 화면 dark overlay, 원형 시계 UI, 질문 번호 `Q5 → Q4 → Q3` 역재생, "Q3 답변 전으로 되감고 있어요" 서브텍스트.
- 완료 직전 카피: "다시 답변할 준비가 되었어요."
- `prefers-reduced-motion: reduce`에서는 짧은 fade 전환으로 대체.

**상태 정책**
- MVP는 **파괴적 되감기** 방식.
- Qn으로 되감으면 Q1~Q(n-1)의 답변/평가는 유지.
- Qn의 기존 답변/평가와 Q(n+1) 이후 답변/평가/최종 리포트는 무효화.
- 현재 질문은 기존 Qn 질문으로 복원하고, 새 답변 제출 뒤부터 새 타임라인으로 진행.

**수용 기준**
- Page 3 완료된 질문 카드에 타임머신 버튼이 보인다.
- 현재 답변 대기 중인 질문에는 버튼을 숨기거나 disabled 처리한다.
- 스트리밍 중, 평가 중, 조기 종료 모달 중에는 실행할 수 없다.
- Qn으로 되감으면 `n-1`개 평가만 남고 Qn 질문이 현재 질문으로 복원된다.
- Page 4에서 Qn 상세 피드백 버튼을 누르면 Page 3의 Qn 답변 전 상태로 이동한다.
- 되감기 후 새 답변을 제출하면 이후 질문과 리포트는 새 타임라인 기준으로 생성된다.

...
| M9 | **타임머신 — 질문 시점 되감기 & 재답변** (F-30) | BE / FE / Design / Feature | **데모 최우선 승격.** 실제 면접 후 "이 질문은 이렇게 답할걸"이라는 후회를 즉시 학습 행동으로 전환하는 핵심 기능. Page 3 질문 카드와 Page 4 상세 피드백에서 특정 질문의 답변 전 상태로 되감아 다시 답변한다. MVP는 파괴적 되감기: 선택 질문 이후 답변/평가/리포트를 무효화하고 새 타임라인으로 진행. |
...
### F-30. 타임머신 — 질문 시점 되감기 & 재답변 [기획 완료, 구현 대기]

...
- 면접관 응답은 **토큰 단위 fade-in** (현재 클라이언트사이드 모사, 백엔드 SSE 미구현)
- 스트리밍 중 끝에 **블록 캐럿** 깜박임
...
* **현재 상태:** v1 (MUI 기반) 그대로 유지. Apple-style 리디자인은 Phase 5 잔여 과제 (`todo.md` 5.2 / `features.md` F-24, F-26 참고)

**현재 구조 (v1)**
- 상단 헤더: "면접 결과 리포트" + "고생하셨습니다…"
- 2컬럼 그리드: 좌 레이더 차트 (Recharts) / 우 종합 피드백 (강점/약점/개선)
- 하단: 문항별 상세 MUI Accordion (점수 아바타 + 질문 + 펼침)
- "새로운 면접 시작하기" 버튼 (세션 초기화 → Page 1)

**v2 적용 예정 변경**
- Hero: "고생하셨습니다. 당신의 종합 점수는" + **카운트업 큰 숫자** + 강점 태그
- 레이더 차트 진입 애니메이션: 동심 다각형 링 → 축선 → 데이터 폴리곤 부풀기 → 꼭지점 스프링 팝 → 라벨 (stagger)
- 범례 바: 0% → 실제값 fill (80ms 간격 stagger)
- 문항별 아코디언: MUI Accordion → `grid-template-rows` 패턴으로 교체
- CSS-only 컴포넌트 분리 (`ReportPage.jsx`), 잔존 MUI 의존성 제거

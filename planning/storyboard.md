...
- "시니어 엔지니어 면접관" + 동적 상태 텍스트 ("답변 생성 중…" / "분석 중…" / "Tech-Interviewer AI · gpt-4.1-mini")
...
**차기 UX 확장안 — 질문 여정 지도 (F-31 기획)**
- 기존 좌측 평가 리스트를 바로 대체하기보다, 먼저 **compact Question Journey Rail** 목업으로 검증한다.
- 면접은 4개 섹션 기반으로 확장 가능하다. 예: `CS Fundamentals`, `Framework / Tech Stack`, `Project Deep Dive`, `Problem Solving / Communication`.
- 각 섹션은 5~7개 질문을 목표로 하며, 질문 타입은 **독립 질문**과 **꼬리 질문** 2개만 유지한다.
- 독립 질문은 새 흐름의 루트 노드, 꼬리 질문은 특정 부모 질문에서 파생된 자식 노드로 표현한다.
- 좌측 rail은 좁기 때문에 질문 전문을 넣지 않는다. 노드, Q번호, 상태, 현재 위치만 보여주는 미니맵 역할에 집중한다.
- 질문 노드 클릭은 면접 진행 변경이 아니라 **읽기 전용 상세 조회**다. 클릭 시 오른쪽 `Question Inspector`가 열리고, 되감기/재답변은 별도 버튼으로만 실행한다.

**Question Inspector 후보 구조**
- Header: `Q08 · 꼬리 질문`, 섹션명, 완료/현재/재답변 상태
- Context: `Q05에서 이어진 질문입니다` + 왜 이 질문이 나왔는지 한 줄 설명
- Body: 질문 전문, 내 답변, 면접관 피드백
- Score: 점수와 Best/Good/Needs Work 톤
- Path: 부모 질문 → 현재 질문 → 자식 질문 요약
- Actions: `대화 위치로 이동`, `이 질문 다시 답변하기`, `닫기`

**Page 3 데스크탑 목업 방향**
```text
┌──────────────┬──────────────────────────────┬──────────────────────┐
│ 질문 흐름     │ 면접 대화                     │ Question Inspector   │
│ ● Q05        │ AI 질문 / 사용자 답변          │ Q08 · 꼬리 질문      │
│  ╰● Q06      │ 입력창                         │ 질문 / 답변 / 피드백 │
│    ╰● Q07    │                              │ 점수 / 연결 경로     │
│      ● Q08   │                              │ [이동] [재답변]      │
└──────────────┴──────────────────────────────┴──────────────────────┘
...
**차기 UX 확장안 — 리포트형 질문 여정 지도**
- Page 4는 면접이 종료된 뒤의 회고 화면이므로, Page 3보다 큰 `Question Journey Map`을 노출할 수 있다.
- 4개 섹션을 lane/group으로 나누고, 각 섹션 안에서 독립 질문과 꼬리 질문의 흐름을 시각화한다.
- 질문 노드 클릭 시 하단 상세 카드 또는 우측 inspector와 기존 문항별 아코디언을 동기화한다.
- 리포트 인사이트 후보: 가장 깊게 파고든 경로, 독립 질문 대비 꼬리 질문 비율, 반복적으로 약했던 섹션, 다음 연습 추천 섹션.
...
선택한 질문 상세: 질문 / 내 답변 / 피드백 / 점수 / 다시 답변하기
```
